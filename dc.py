...
class Booster:
    def __init__(self) -> None:
        ...
        self.config         = json.load(open('./data_discord/config.json', 'r'))
        self.proxies        = open('./data_discord/proxies.txt', 'r').read().splitlines()
        if not len(self.proxies):
            ...
            time.sleep(10)
            os._exit(0)
        self.usernames      = open('./data_discord/usernames.txt', 'r',encoding='utf8').read().splitlines()

        
        self.threads        = self.config['threads']
        self.capmonster_key = self.config['solver_key']
    ...
    def __gen__(self, captcha_token: str, proxy: str) -> None:
        try:
            ...
            token = r.json()['token']
            self.captcha_tokens.append(captcha_token)
            with open("./data_discord/tokens.txt", "a") as x: x.write(f'{token}\n')
            with open("./data_discord/accounts.txt", "a") as x: x.write(f'{email}:{self.config["password"]}:{token}\n')
    ...
    def __captcha__(self):
        while True:
            try:
                proxy = random.choice(self.proxies)
                capmonster = HCaptchaTask(self.config["solver_key"])
