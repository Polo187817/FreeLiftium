...
while accNo > i:
    ...
    first_name = (random.choice(open("./data_ig/Fnames.txt").read().split()))
    last_name =  (random.choice(open("./data_ig/Lnames.txt").read().split()))
    full_name = (first_name + ' ' + last_name)
    username = (first_name + last_name + '.' + str(random.randint(1, 100)) + str(random.randint(1, 1000)))
    password = (open("./data_ig/password.txt").readline())
    ...
    try:
        ...
        next1.click()
    sleep(10)
    with open('./data_ig/username.txt', 'w') as f_output:
